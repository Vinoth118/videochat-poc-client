...
const CustomerLogin: NextPage = () => {
    ...
    const onLogin = async (data: NewUser) => {
        ...
        try {
            const res = await axios.post('/login', { email: data.email, type: 'customer' });
    ...
    const onRegister = async (data: NewUser) => {
        setSubmitLoading(true);
        try {
            const res = await axios.post('/register', { email: data.email, name: data.name, type: 'customer' });
            if(res.data && res.data.success) {
                onLoginSuccess(res.data.data);
                setSubmitLoading(false);
                return true;
